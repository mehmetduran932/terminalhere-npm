...
const backgroundPaths = ['Software\\Classes\\Directory\\Background\\shell', 'Software\\Classes\\Drive\\Background\\shell'];
...
export class ContextMenuService {
  ...
  async enable(target: TerminalTarget): Promise<TerminalStatus> {
    ...
    if (!app.executablePath) throw new Error(`${app.displayName} could not be found on this computer.`);
  ...
  async disable(target: TerminalTarget): Promise<TerminalStatus> {
    const app = await detectApp(target);
    for (const parent of [...backgroundPaths, ...directoryPaths]) await this.registry.deleteTree(`${parent}\\${app.registryKeyName}`);
  ...
  async enableAll(): Promise<TerminalStatus[]> {
    ...
    for (const target of targets) {
      try { results.push(await this.enable(target)); } catch { results.push(await this.status(target)); }
    }
    return results;
  }

  async disableAll(): Promise<TerminalStatus[]> { return Promise.all(targets.map(target => this.disable(target))); }
  async allStatuses(): Promise<TerminalStatus[]> { return Promise.all(targets.map(target => this.status(target))); }

  private async writeEntry(parent: string, app: Awaited<ReturnType<typeof detectApp>>, token: '%V' | '%1'): Promise<void> {
